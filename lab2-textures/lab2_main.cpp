...
int mag = GL_LINEAR;
int mini = GL_NEAREST_MIPMAP_NEAREST;
...
GLuint		positionBuffer, colorBuffer, indexBuffer, vertexArrayObject, texCoords, texture, texcoordsExplo, exploArrayObject, exploPosition, explo, indexBuffer2;
...
void initGL()
{
	...
	glGenBuffers(1, &exploPosition);								// Create a handle for the vertex position buffer
	glBindBuffer(GL_ARRAY_BUFFER, exploPosition);					// Set the newly created buffer as the current one
	glBufferData(GL_ARRAY_BUFFER, sizeof(positionsExplo), positionsExplo, GL_STATIC_DRAW);		// Send the vetex position data to the current buffer
	glVertexAttribPointer(0, 3, GL_FLOAT, false/*normalized*/, 0/*stride*/, 0/*offset*/);
	glEnableVertexAttribArray(0);

	float texcoordsexplo[] = {
	...
	glGenBuffers(1, &texcoordsExplo);
	glBindBuffer(GL_ARRAY_BUFFER, texcoordsExplo);
	glBufferData(GL_ARRAY_BUFFER, sizeof(texcoordsexplo), texcoordsexplo, GL_STATIC_DRAW);
	...
	glGenTextures(1, &explo);

	glBindTexture(GL_TEXTURE_2D, explo);
