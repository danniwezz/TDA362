...
GLuint		positionBuffer, colorBuffer, indexBuffer, vertexArrayObject, texcoordsBuffer, texture, explosion, exploPosBuffer, exploArrayObject, explocoordBuffer, exploIndexBuffer;
...
void initGL()
{
	...
	glGenVertexArrays(1, &exploArrayObject);
	glBindVertexArray(exploArrayObject);

	//exploArrayObject

	const float exploPositions[] = {
		// X Y Z
		3.0f, 3.0f, -30.0f,    // v0
		3.0f, -3.0f, -30.0f,   // v1
		-3.0f, -3.0f, -30.0f,   // v2
	...
	glGenBuffers(1, &exploPosBuffer);													// Create a handle for the vertex position buffer
	glBindBuffer(GL_ARRAY_BUFFER, exploPosBuffer);									// Set the newly created buffer as the current one
	glBufferData(GL_ARRAY_BUFFER, sizeof(exploPositions), exploPositions, GL_STATIC_DRAW);		// Send the vetex position data to the current buffer
	glVertexAttribPointer(0, 3, GL_FLOAT, false/*normalized*/, 0/*stride*/, 0/*offset*/);
	glEnableVertexAttribArray(0);
	...
	glGenBuffers(1, &explocoordBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, explocoordBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(explocoords), explocoords, GL_STATIC_DRAW);
	...
	// Create the element array buffer object
	///////////////////////////////////////////////////////////////////////////	
	const int exploIndices[] = {
		0, 1, 3, // Triangle 1
		1, 2, 3  // Triangle 2
	};
	glGenBuffers(1, &exploIndexBuffer);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, exploIndexBuffer);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(exploIndices), exploIndices, GL_STATIC_DRAW);
